...
final case class Bounds(width: Double, height: Double):
  ...
  def contains(position: P2d): Boolean = position.x >= 0 && position.x <= width && position.y >= 0 &&
    position.y <= height

  def clamp(position: P2d): P2d = P2d(x = position.x.max(0).min(width), y = position.y.max(0).min(height))
