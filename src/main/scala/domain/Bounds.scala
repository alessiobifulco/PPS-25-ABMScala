...
final case class Bounds(width: Double, height: Double):

  def contains(position: P2d): Boolean =
    position.x >= 0 && position.x <= width && position.y >= 0 && position.y <= height

  def clamp(position: P2d): P2d =
    P2d(x = position.x.max(0).min(width), y = position.y.max(0).min(height))
  ...
  def bounce(position: P2d, velocity: V2d): (P2d, V2d) =
    ...
    val correctedVelocity = V2d(x = if shouldBounceX then -velocity.x else velocity.x, y = if shouldBounceY then -velocity.y else velocity.y)
