...
final case class CircularSpace(center: P2d, radius: Double) extends Space:
  ...
  override def clamp(position: P2d): P2d =
    ...
    if distance <= radius then
      ...
      P2d(x = center.x + offset.x / distance * radius, y = center.y + offset.y / distance * radius)
    ...
    if distance == 0 then
      ...
      val correctedVelocity =
        if shouldBounce then
          V2d(x = velocity.x - 2 * velocityAlongNormal * outwardNormal.x, y = velocity.y - 2 * velocityAlongNormal * outwardNormal.y)
        else
          velocity
  ...
  override def stop(position: P2d, velocity: V2d): (P2d, V2d) =
    ...
    val correctedPosition = clamp(position)
    val correctedVelocity =
      if isOutside || isMovingOutward then
        V2d.zero
      else
        velocity
  ...
  private def relativeToCenter(position: P2d): V2d =
    V2d(x = position.x - center.x, y = position.y - center.y)